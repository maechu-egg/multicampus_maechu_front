...
const parseMealPlan = (dietPlan : string): MealPlanData => {
    ...
    try {
        ...
        const planData = typeof dietPlan === 'string'
            ? JSON.parse(dietPlan)
            : dietPlan;
...
const DietPlanSection: React.FC < {
                ...
                if (line.includes('간식 옵션:')) {
                    ...
                    lines.forEach((snackLine : string) => {
                        if (snackStarted && snackLine.startsWith('*') && !snackLine.includes('**')) {
                            const snackOption = snackLine
                                .replace('*', '')
                                .trim();
                            if (snackOption && !snackOption.includes('열량:') && !snackOption.includes('단백질:') && !snackOption.includes('탄수화물:') && !snackOption.includes('지방:')) {
                                meals
                                    .snack
                                    .foods
                                    .push(snackOption);
    ...
    const hasData = Object
        .values(meals)
        .some((meal : MealData) => meal.foods.length > 0 || meal.amounts.length > 0);

    if (!hasData) {
        return (
            <ResultSection><SectionTitle><SectionIcon> 🍽️</SectionIcon> 맞춤형 식단 계획</SectionTitle><SectionContent> < div style = {{ textAlign: 'center', padding: '20px' }} > 데이터를 불러오는 중입니다 ...</div></SectionContent></ResultSection>
        );
    }
  ...
  `;
    ...
    const TimelineMealCard = styled.div < {
  ...
  `;
    ...
    const TimelineMealHeader = styled.div < {
        $backgroundColor: string
    } > `
    background: ${props => props.$backgroundColor};
    padding: 15px;
  ...
  `;
    ...
    return (
        <ResultSection><SectionTitle><SectionIcon> 🍽️</SectionIcon> 맞춤형 식단 계획</SectionTitle><SectionContent><TimelineContainer><MealTimelineGrid> {/* 아침 식사 */
        } < TimelineMealCard $backgroundColor = "#FF9800" > <TimelineMealHeader $backgroundColor = "#FF9800" > <DietMealIcon> 🌅</DietMealIcon><MealTitle> 아침</MealTitle></TimelineMealHeader><TimelineMealContent><TimelineFoodList> {
            meals
                .breakfast
                .foods
                .map((food : string, index : number) => (< TimelineFoodItem key = {
                    index
                } > <DietFoodIcon> 🍳</DietFoodIcon><FoodDetails><DietFoodName> {
                    food
                }</DietFoodName><FoodAmount> {
                    meals
                        .breakfast
                        .amounts[index]
                }</FoodAmount></FoodDetails></TimelineFoodItem>))
        }</TimelineFoodList></TimelineMealContent></TimelineMealCard> {/* 점심 식사 */
        } < TimelineMealCard $backgroundColor = "#4CAF50" > <TimelineMealHeader $backgroundColor = "#4CAF50" > <DietMealIcon> ☀️</DietMealIcon><MealTitle> 점심</MealTitle></TimelineMealHeader><TimelineMealContent><TimelineFoodList> {
            meals
                .lunch
                .foods
                .map((food : string, index : number) => (< TimelineFoodItem key = {
        ...
        }</TimelineFoodList></TimelineMealContent></TimelineMealCard> {/* 저녁 식사 */
        } < TimelineMealCard $backgroundColor = "#2196F3" > <TimelineMealHeader $backgroundColor = "#2196F3" > <DietMealIcon> 🌙</DietMealIcon><MealTitle> 저녁</MealTitle></TimelineMealHeader><TimelineMealContent><TimelineFoodList> {
            meals
                .dinner
                .foods
                .map((food : string, index : number) => (< TimelineFoodItem key = {
        ...
        }</TimelineFoodList></TimelineMealContent></TimelineMealCard> {/* 간식 */
        } {
            meals.snack.foods.length > 0 && (
                < TimelineMealCard $backgroundColor = "#9C27B0" > <TimelineMealHeader $backgroundColor = "#9C27B0" > <DietMealIcon> 🍎</DietMealIcon><MealTitle> 간식</MealTitle></TimelineMealHeader><TimelineMealContent><TimelineFoodList> {
                    meals
                        .snack
                        .foods
                        .map((food : string, index : number) => (< TimelineFoodItem key = {
                ...
                }</TimelineFoodList></TimelineMealContent></TimelineMealCard>
            )
        }</MealTimelineGrid></TimelineContainer></SectionContent></ResultSection>
    );
};
...
const CautionSection: React.FC < {
    ...
    return (
        <ResultSection><SectionTitle><SectionIcon> ⚠️</SectionIcon> 주의 사항</SectionTitle><SectionContent><CautionList> {
            cautions.map((caution, index) => (< CautionItem key = {
                index
            } > <CautionIcon> ⚠️</CautionIcon><CautionText> {
                caution
            }</CautionText></CautionItem>))
        }</CautionList></SectionContent></ResultSection>
    );
};
...
function DietPage() {
            ...
            if (memberId !== undefined && state.token) {
                try {
                    ...
                    const response = await axios.get(
                        'http://localhost:8001/record/diet/calculate/tdee',
                        {
                            params: {
                                member_id: memberId
                            },
    ...
    return (
        <Container><Header><Title> 오늘의 식단</Title><InfoContainer> {
            data
                ? (
                    <><TotalCalories> Total
                    : {
                    ...
                    kcal</TotalCalories><CurrentDate> {
                        new Date().toLocaleDateString('ko-KR')
                    } {
                        new Date().toLocaleString('ko-KR', {weekday: 'long'})
                    }</CurrentDate></>
                )
                : (<TotalCalories> 로딩 중 ...</TotalCalories>)
        }</InfoContainer></Header><GoalContainer> {
            data
                ? (<><Goal> {
                    data.weight
                }
                ...
                kcal</Goal><Goal> 추천 단백질
                : {
                    data.recommendedProtein
                }
                g</Goal><Goal> 추천 탄수화물
                : {
                    data.recommendedCarb
                }
                g</Goal><Goal> 추천 지방
                : {
                    data.recommendedFat
        ...
        }</GoalContainer> {/* 메인 화면의 식단 추천 버튼 */
        } < RecommendationButton onClick = {
            handleRecommendationClick
        } > 식단 추천</RecommendationButton><RecordList> {
            dietRecords.map((record, index) => (< RecordItem key = {
                index
            }
            onClick = {
        ...
        }</RecordList> {/* 첫 번째 모달 - 입력 폼 */
        } {
            isModalOpen && (
                <ModalOverlay><ModalContent><ModalHeader><h2> 식단 추천</h2> < CloseButton onClick = {
                    closeModal
                } > X</CloseButton></ModalHeader><ModalBody><InputContainer><label> 목표 칼로리
                : </label> < Input type = "number" value = {
                    data
                        ?.recommendedCalories || 0
                }
                readOnly /> <label> 재료(쉼표로 구분): </label> < Input type = "text" value = {
                    ingredients
                }
                onChange = {
                    (e) => setIngredients(e.target.value)
                } /> <label> 식 제한(쉼표로 구분): </label> < Input type = "text" value = {
                    dietaryRestrictions
                }
                onChange = {
                    (e) => setDietaryRestrictions(e.target.value)
                } /> <label> 알레르기(쉼표로 구분): </label> < Input type = "text" value = {
                    allergies
                }
                onChange = {
                    (e) => setAllergies(e.target.value)
                } /> <label> 의료 조건(쉼표로 구분): </label> < Input type = "text" value = {
                    medicalConditions
                }
                onChange = {
                    (e) => setMedicalConditions(e.target.value)
                } /> <label> 하루 식사 횟수 : </label> < Input type = "text" value = {
                    mealsPerDay
                }
                onChange = {
                    (e) => setMealsPerDay(e.target.value)
                } /> <label> 조리 난이도(쉼표 구분): </label> < Input type = "text" value = {
                    cookingPreference
                }
                onChange = {
                    (e) => setCookingPreference(e.target.value)
                } /> <RecommendButton onClick = {
                    handleRecommend
                } > 추천받기</RecommendButton></InputContainer></ModalBody></ModalContent></ModalOverlay>
            )
        } {/* 두 번째 모달 - 결과 표시 */
        } {
            isResultModalOpen && (
                <ModalOverlay><ModalContent><ModalHeader><h2> 맞춤형 식단 계획</h2> < CloseButton onClick = {
                    closeResultModal
                } > ×</CloseButton></ModalHeader><ModalBody> {
                    dietPlan
                        ? (<> < DietPlanSection dietPlan = {
                            dietPlan
                        } /> <CautionSection dietPlan = {
                            dietPlan
                  ...
                  padding: '40px',
                  color: '#666'
                }} > <div style = {{ fontSize: '24px', marginBottom: '20px' }} > 🔄</div><p> 맞춤형 식단을 생성하고 있습니다 ...</p></div>
                        )
